...
Guarantees the badge inventory scan completes on the local-data fast path when no badge-detail requests are needed, so fully-covered users never hit a startup crash.
...
- **WHEN** all badges resolve from local card data (zero pending detail fetches)
...
#### Scenario: Cancellation flag available on fast path
