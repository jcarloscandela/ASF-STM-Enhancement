...
describe("mergeWithDefaults (scanner config)", () => {
  it("keeps a stored legacy inventoryScan:true across reload (preserved, never read)", () => {
  ...
  it("falls back per key on a wrong-typed value while preserving the rest", () => {
    ...
    const merged = mergeWithDefaults(stored, realDefaults()) as Config;
    assert.equal(merged.matchFriends, false, "string where boolean expected falls back");
    ...
    assert.equal(merged.autoSend, true, "valid stored value survives");
  });
...
describe("resolveScanPlan precedence (scanner config)", () => {
  ...
  it("defaults to the inventory scan regardless of a legacy stored flag", () => {
    assert.equal(resolveScanPlan(withFilters({ inventoryScan: false, useScanFilters: false })).mode, "inventory");
  });
