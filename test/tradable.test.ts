...
describe("buildScanEligibility", () => {
  it("flags evenly distributed badges as balanced", () => {
    const result = buildScanEligibility({ 753: { A: 2, B: 2, C: 2, D: 2, E: 2 } }, { 753: { size: 5, name: "Game" } });
  ...
  it("flags unevenly distributed badges as unbalanced", () => {
    const result = buildScanEligibility({ 753: { A: 3, B: 1, C: 1, D: 1, E: 1 } }, { 753: { size: 5, name: "Game" } });
    assert.equal(result[753]?.unbalanced, true);
  });

  it("flags badges with missing cards as unbalanced once a set exists", () => {
    const result = buildScanEligibility({ 753: { A: 2, B: 2 } }, { 753: { size: 5, name: "Game" } });
    assert.equal(result[753]?.unbalanced, true);
  });

  it("skips games absent from the badges database", () => {
    const result = buildScanEligibility({ 12345: { A: 5 } }, { 753: { size: 5, name: "Game" } });
